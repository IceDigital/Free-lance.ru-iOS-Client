...
- (void)reloadData
{
    ...
    if (self.bubbleDataSource && (count = [self.bubbleDataSource rowsForBubbleTable:self]) > 0)
    {
        ...
        NSDate *last = nil;//[NSDate dateWithTimeIntervalSince1970:0];
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (last==nil || [data.date timeIntervalSinceDate:last] < -self.snapInterval)
            {
#if !__has_feature(objc_arc)
...
{
	[(IDMessagesViewController *)self.bubbleDataSource scrollViewDidScroll:scrollView];
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.row>0)
    {
        ...
        {
            row+=[self numberOfRowsInSection:i]-1;
        }
        row+=indexPath.row-1;
        [(IDMessagesViewController *)self.bubbleDataSource messageRowSelected:row];
    }
}
